...
class Att2all2Core(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        other_input_sums = self.i2h(xt) + self.h2h(state[0][-1])
        ...
        if self.is_hard_attention:
            other_input_sums = expand_tensor(other_input_sums, att_size)
            h = expand_tensor(state[0].view(batch_size, self.rnn_size), att_size)
            c = expand_tensor(state[1].view(batch_size, self.rnn_size), att_size)
            state = (h.unsqueeze(0), c.unsqueeze(0))

            att_res = att_feats.view(-1, att_feat_size)  # (batch * att_size) * att_feat_size
            output, state = self._core_func(other_input_sums, att_res, state)
            predicts = F.softmax(self.logit(output), dim=-1).view(batch_size, att_size, -1)
            prob = torch.bmm(weight.unsqueeze(1), predicts).squeeze(1)
            log_prob = torch.log(prob + 1e-10)
    ...
    def _core_func(self, other_input_sums, att_res, state):
        all_input_sums = other_input_sums + self.a2h(att_res)
        sigmoid_chunk = all_input_sums.narrow(1, 0, 3 * self.rnn_size)
        sigmoid_chunk = F.sigmoid(sigmoid_chunk)
        in_gate = sigmoid_chunk.narrow(1, 0, self.rnn_size)
        forget_gate = sigmoid_chunk.narrow(1, self.rnn_size, self.rnn_size)
